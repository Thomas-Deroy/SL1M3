...
bool GameEngine::DrawBitmap(const Bitmap* bitmapPtr, int left, int top, RECT srcRect, int destW, int destH) const
{
	if (m_IsPainting)
	{
		if (!bitmapPtr->Exists()) return false;
		...
		const int opacity = bitmapPtr->GetOpacity();
		if (opacity == 0 && bitmapPtr->HasAlphaChannel()) return true;

		HDC hdcMem = CreateCompatibleDC(m_HdcDraw);
		HBITMAP hbmOld = (HBITMAP)SelectObject(hdcMem, bitmapPtr->GetHandle());

		// Calculate Source Width/Height from the rect
		int srcW = srcRect.right - srcRect.left;
		int srcH = srcRect.bottom - srcRect.top;

		if (bitmapPtr->HasAlphaChannel())
		{
			BLENDFUNCTION blender = { AC_SRC_OVER, 0, (BYTE)(2.55 * opacity), AC_SRC_ALPHA };

			AlphaBlend(m_HdcDraw, left, top, destW, destH, hdcMem, srcRect.left, srcRect.top, srcW, srcH, blender);
		}
		else
		{
			// TransparentBlt also supports scaling
			TransparentBlt(m_HdcDraw, left, top, destW, destH, hdcMem, srcRect.left, srcRect.top, srcW, srcH, bitmapPtr->GetTransparencyColor());
		}
...
bool Font::LoadFile(const tstring& filePath)
{
	int result = AddFontResourceEx(filePath.c_str(), FR_PRIVATE, 0);
	return (result > 0);
}
